...
# There are 900 images in the training data.
#
# There are 1 images in the Set5 test data.
#
# There are 8 images in the Set14 test data.
#
# There are 100 images in the Urban100 test data.
...
def convert_image(img, source, target):
    ...
    每种格式的含义
    格式	描述	典型应用
    '[0, 255]'	原始像素值范围 (0-255)	未处理的原始图像
    '[0, 1]'	归一化到0-1范围 (除以255)	基本归一化处理
    '[-1, 1]'	归一化到-1到1范围	GANs常用范围
    'imagenet-norm'	ImageNet数据集专用归一化	迁移学习常用
    '''
    # ImageNet专用归一化的具体实现:当选择 'imagenet-norm' 时，通常会这样处理：
    # # ImageNet均值和标准差
    # imagenet_mean = [0.485, 0.456, 0.406]
    # imagenet_std = [0.229, 0.224, 0.225]
    # 归一化转换
    # normalize = transforms.Normalize(mean=imagenet_mean, std=imagenet_std)
    # img = transforms.ToTensor()(img)  # 先转为[0,1]范围
    # img = normalize(img)  # 应用ImageNet归一化
...
class ImageTransforms(object):
    ...
    def __call__(self, img):
        ...
        # 应用缩放因子4后：
        #
        # 生成的低分辨率图像（LR）：100×100 像素
        #
        # 模型需要学习如何将这个100×100的图像还原为400×400的图像
