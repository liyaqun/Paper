...
# SRDataset
crop_size = 96  # 对目标 HR 图像的裁剪大小 96*96
# 高分辨率图像是大小随机的块 96 × 96 来自训练图像。HR 图像用作训练 SRGAN 的生成器的目标，并作为 SRGAN 的鉴别器的输入。
...
n_channels_g = 32  # number of channels in-between, i.e. the input and output channels for the residual and subpixel convolutional blocks
n_blocks_g = 8  # number of residual blocks
...
n_channels_d = 32  # number of output channels in the first convolutional block, after which it is doubled in every 2nd block thereafter
...
# HR 图像的低分辨率版本由 4 倍双三次下采样生成。LR 图像是 SRResNet 和 SRGAN 生成器的输入。
# 超分辨（SR）图像
# 超分辨率图像是低分辨率图像的智能放大版本。超分辨率图像是 SRResNet 和 SRGAN 生成器的输出，用于与目标高分辨率图像进行比较，同时也用作 SRGAN 鉴别器的输入。

# 由于鉴别器是一个二值图像分类器，使用每个 LR 图像的 SR 和 HR 对应部分进行训练，因此标签是 1(natural origin) 或者 0(artificial, Generator origin)
...
def train(train_loader, generator, discriminator, truncated_vgg19, content_loss_criterion, adversarial_loss_criterion,
    ...
    for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
        ...
        # 为啥要和全为1 的进行对比？？？ --> 在训练GAN时，需要为真实图像创建标签"1"
