...
import torch
from AutoLUT.MuLUT_AutoLUT.common.utils import _rgb2ycbcr, PSNR, cal_ssim, modcrop
from AutoLUT.MuLUT_AutoLUT.new.model_transfer_to_lut import TestOptions

sys.path.insert(0, "../")  # run under the current directory


# 4D equivalent of triangular interpolation, faster version
# 四维单纯形插值：在4D空间执行单纯形插值，实现高效图像上采样
def FourSimplexInterpFaster(weight, img_in, h, w, interval, rot, upscale=4, mode=0):
    q = 2 ** interval
    L = 2 ** (8 - interval) + 1
...
class eltr:
    def __init__(self, dataset, opt, lutDict):
        ...
        files.sort()
        print(f"数据集 {dataset} 下的图像数量：{len(files)}")  # 查看有多少个文件
        # 加载高分辨率图像
    ...
    def run(self, num_worker=24):
        ...
        for i in range(len(self.files)):
            result = self._worker(i)
            psnr_ssim_s.append(result)

            # 计算并打印平均指标
            psnr_values = [result[0] for result in psnr_ssim_s]
            ssim_values = [result[1] for result in psnr_ssim_s]
            print('Dataset {} | AVG LUT PSNR: {:.2f} SSIM: {:.4f}'.format(
                self.dataset, np.mean(psnr_values), np.mean(ssim_values)))

    def _worker(self, i):
        print(f"{self.opt.testDir}")
        img_lr = np.array(Image.open(
            os.path.join(self.opt.testDir, dataset, 'LR_bicubic/X4'.format(self.opt.scale), self.files[i]))).astype(
            np.float32)
        print(f"低分辨率图像的形状：{img_lr.shape}")
        ...
        print(f"低分辨率图像的形状：{img_lr.shape},高分辨率图像的形状:{img_gt.shape}")
        # 低分辨率图像的形状：(128, 128, 3),高分辨率图像的形状:(170, 170, 3)
        img_gt = modcrop(img_gt, self.opt.scale)


        print(f"低分辨率图像的形状：{img_lr.shape},高分辨率图像的形状：{img_gt.shape}")
        # 低分辨率图像的形状：(128, 128, 3),高分辨率图像的形状：(168, 168, 3)
        ...
        for s in range(self.opt.stages):
            ...
            for mode in range(len(self.opt.modes)):
                key = "s{}_{}".format(str(s), mode)
                print(f"key:{key}")
                if mode in [1, 2]:
                    pad = (0, 2)
                else:
        ...
        Image.fromarray(img_out).save(
            os.path.join(self.result_path, '{}_{}_{}bit.png'.format(self.files[i].split('/')[-1][:-4], self.opt.lutName,
                                                                    8 - self.opt.interval)))
        print(f"img_gt：{img_gt.shape},img_out：{img_out.shape}")  # img_gt：(168, 168, 3),img_out：(512, 512, 3)
        # img_out 是超分模型对低分辨率图像（LR） 上采样后的结果，其尺寸由 LR 图像的尺寸和缩放因子（scale）决定：
        # img_out尺寸 = LR图像尺寸 × scale
        y_gt, y_out = _rgb2ycbcr(img_gt)[:, :, 0], _rgb2ycbcr(img_out)[:, :, 0]

        print(f"y_gt：{y_gt.shape},y_out：{y_out.shape}")
        # y_gt：(168, 168),y_out：(512, 512)
...
if __name__ == "__main__":
    ...
    for s in range(opt.stages):
        if (s + 1) == opt.stages:
            ...
            v_num = 1
        for mode in range(len(opt.modes)):
            key = "s{}_{}".format(str(s), mode)
            # 加载lut表格
            lutPath = os.path.join("D:/pycharm/LUT/LUT/AutoLUT/MuLUT_AutoLUT/models/debug/expr_6",
            ...
            print(f"Loaded {lutPath}")
            print(f"lutDict：{lutDict}")
